...
class MatchGame extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.timeRanges, 1000)
  }
  ...
  onClickChangeTab = tabs => {
    this.setState({category: tabs})
  }

  imageChange = thumbnailUrl => {
    ...
    if (image === imageUrl) {
      ...
      this.setState(prevState => ({score: prevState.score + 1, image: newUrl}))
    } else {
      clearInterval(this.timerId)
      this.setState({isTrue: true})
    }
  ...
  renderWindows = () => {
    ...
    const filters = imagesList.filter(each => each.category === category)
    return (
      <>
        <img src={image} alt="match" className="match-image" />
        <ul className="heading-container">
          {tabsList.map(each => (
            <li className="list-container" key={each.tabId}>
              <button
                className={`heading-description ${
                  each.tabId === category ? 'active' : ''
                }`}
                type="button"
                onClick={() => this.onClickChangeTab(each.tabId)}
              >
                {each.displayText}
              </button>
            </li>
  ...
  render() {
    const {isTrue, timer, score} = this.state
    return (
      <>
        <Header timer={timer} score={score} />
        <div className="match-container">
          {isTrue ? this.renderWinOrLoss() : this.renderWindows()}
        </div>
      </>
    )
  }
